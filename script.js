...
// 將輸出工作表的每個儲存格強制轉成文字型態（避免 Excel 轉科學記號）
...
(function () {
  ...
  function process(buffer, originalName) {
    ...
    const outWs = forceTextFormat(XLSX.utils.aoa_to_sheet(out));
    ...
    render({
      originalName, colA: colA.length, colB: colB.length, colC: colC.length, colD: colD.length,
      imei, mac
    });
  }
